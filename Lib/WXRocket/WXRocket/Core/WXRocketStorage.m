...
NSTimeInterval kWXRocketLogExpiredTime = 7 * 24 * 60 * 60;
NSString *const kWXRocketCollectionKeyValueRecordsFileName = @"records";
NSUInteger kWXRocketLogStoreMaxLength = 16 * 1024;

@interface WXRocketStorage ()
...
@implementation WXRocketStorage

+ (instancetype)shared {
    static WXRocketStorage *_sharedInstance = nil;
...
- (instancetype)init {
    if ((self = [super init])) {
        _storeDirectory = [WXRocketUtility currentStorePath];
        [self rebuildRocketStorageDirectoryIfNeed];
...
#ifdef DEBUG
    NSAssert(lineLength < kWXRocketLogStoreMaxLength, @"line length should less than 16KB");
#endif
...
- (NSUInteger)readKeyValuesInCollection:(NSString *)collection
    ...
    dispatch_sync(self.storeQueue, ^(void) {
        fileContent = [self loadMmapContentAtDirectory:self.storeDirectory fileName:kWXRocketCollectionKeyValueRecordsFileName];
    });
...
- (void)readKeyValuesInPageRange:(NSRange)pageRange
                    inCollection:(NSString *)inCollection
                      ...
                      usingBlock:(nonnull WXRocketStoragePageFilterBlock)usingBlock {
    ...
    dispatch_sync(self.storeQueue, ^(void) {
        pageString = [self loadStringContentAtDirectory:self.storeDirectory
                                               fileName:kWXRocketCollectionKeyValueRecordsFileName
                                              pageRange:pageRange];
    });
    ...
    NSMutableArray<WXRocketStorageObject *> *objects = [NSMutableArray array];
    [pageString enumerateLinesUsingBlock:^(NSString *_Nonnull line, BOOL *_Nonnull stop) {
        ...
        if (parts.count == 3 && parts[0] && parts[1].length && parts[2].length) {
            [objects addObject:[WXRocketStorageObject createObjectWithCollection:parts[0] key:parts[1] string:parts[2]]];
        }
    }];

    NSArray<WXRocketStorageObject *> *sortObject = objects;
    if (orderBy == kWXRocketStorageOrderTypeDescending) {
    ...
    for (int i = 0; i < sortedPageCount; i++) {
        ...
        NSArray<WXRocketStorageObject *> *objects = [sortObject subarrayWithRange:NSMakeRange(loc, length)];
        if (inCollection) {
            NSMutableArray<WXRocketStorageObject *> *collections = [NSMutableArray array];
            [objects enumerateObjectsUsingBlock:^(WXRocketStorageObject *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)autoTrimExpiredLog {
    ...
    [logDirectories enumerateObjectsUsingBlock:^(NSString *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
        ...
        if (createDate && createDate.timeIntervalSinceNow < -kWXRocketLogExpiredTime) {
            [[NSFileManager defaultManager] removeItemAtPath:[rocketPath stringByAppendingPathComponent:obj] error:NULL];
...
- (void)rebuildRocketStorageDirectoryIfNeed {
    NSError *error;
    NSString *dir = self.storeDirectory;
    // create directory if not exist
    if (![[NSFileManager defaultManager] fileExistsAtPath:dir]
        && ![[NSFileManager defaultManager] createDirectoryAtPath:dir withIntermediateDirectories:YES attributes:nil error:&error]) {
        MTHLogWarn(@"[rocket][persistance] create directory %@ failed, %@ %@", dir, @(error.code), error.localizedDescription);
    }
}
...
- (dispatch_queue_t)storeQueue {
    ...
    dispatch_once(&onceToken, ^{
        ...
        self->_storeQueue = dispatch_queue_create("com.tt.rocket.storage", attr);
    });
    return _storeQueue;
}
...
- (MTAppenderFile *)collectionKeyValueFile {
    if (_collectionKeyValueFile == nil) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            MTAppenderFile *file = [[MTAppenderFile alloc] initWithFileDir:self.storeDirectory name:kWXRocketCollectionKeyValueRecordsFileName];
            [file open];
            ...
            // launch time
            NSString *launchTime = [NSString stringWithFormat:@"%@", @([WXRocketUtility appLaunchedTime])];
