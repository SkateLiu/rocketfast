...
@interface WXRocketWebViewController () <WKUIDelegate, WKNavigationDelegate>
...
@implementation WXRocketWebViewController
...
- (void)loadWebViewWithText:(NSString *)text {
    NSString *htmlString = [NSString stringWithFormat:@"<head><meta name='viewport' content='initial-scale=1.0'></head><body><pre>%@</pre></body>", [self stringByEscapingHTMLEntitiesInString:text]];
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    if (!navigationAction.targetFrame) {
        ...
        if (url) {
            WXRocketWebViewController *webVC = [[[self class] alloc] initWithURL:url];
