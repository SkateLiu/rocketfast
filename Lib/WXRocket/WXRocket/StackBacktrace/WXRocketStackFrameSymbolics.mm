//
//  WXRocketStackFrameSymbolics.mm
//  WXRocket
//

#import "WXRocketStackFrameSymbolics.h"
...
static inline int qcompare(const void *a, const void *b) {
    if ((*(wxrocket_dyld_image *)a).beginAddr > (*(wxrocket_dyld_image *)b).beginAddr) {
        return 1;
    } else {
        return -1;
    }
...
WXRocketStackFrameSymbolics::~WXRocketStackFrameSymbolics() {
    if (allImageInfo) {
        free(allImageInfo);
        allImageInfo = NULL;
    }
...
static void load_dyld_images(wxrocket_dyld_image *&allImageInfo, uint32_t &imageInfoCount) {
    uint32_t count = _dyld_image_count();
    allImageInfo = (wxrocket_dyld_image *)malloc(count * sizeof(wxrocket_dyld_image));
    imageInfoCount = 0;
    ...
    qsort(allImageInfo, count, sizeof(wxrocket_dyld_image), qcompare);
}
...
uint32_t WXRocketStackFrameSymbolics::getImageInfoIndexByAddress(const uintptr_t address) {
    if (imageInfoCount == 0) {
        return UINT_MAX;
    }
...
bool WXRocketStackFrameSymbolics::getDLInfoByAddr(vm_address_t addr, Dl_info *const info, bool slide) {
