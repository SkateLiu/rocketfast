...
#import "WXRocketHooking.h"
#import "WXRocketLogMacros.h"

@implementation WXRocketHooking

+ (SEL)swizzledSelectorForSelector:(SEL)sel {
    return NSSelectorFromString([NSString stringWithFormat:@"_rocket_swizzle_%x_%@", arc4random(), NSStringFromSelector(sel)]);
}

+ (SEL)swizzledSelectorForSelectorConstant:(SEL)sel {
    return NSSelectorFromString([NSString stringWithFormat:@"_rocket_swizzle_%@", NSStringFromSelector(sel)]);
}
