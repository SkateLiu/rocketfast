...
#import "WXRocketLFUCache.hpp"

WXRocketLFUCache::WXRocketLFUCache(int capacity) {
    cur_cap = 0;
    max_cap = capacity;
    m_list.emplace_front(pair<int, list<KeyType>>(1, list<KeyType>())); // 插入 count == 1 的节点
}

WXRocketLFUCache::~WXRocketLFUCache() {
    m_list.clear();
    u_map.clear();
}

void WXRocketLFUCache::right_move(LFUMapValue *value) {
...
ValueType WXRocketLFUCache::get(KeyType key) {
...
void WXRocketLFUCache::put(KeyType key, ValueType value) {
    if (max_cap == 0) {
        return;
    }
    map_it = u_map.find(key);
...
void WXRocketLFUCache::remove_key(KeyType key) {
