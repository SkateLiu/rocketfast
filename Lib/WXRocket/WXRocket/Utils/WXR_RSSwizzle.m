...
static BOOL blockIsAnImpFactoryBlock(id block) {
    const char *blockType = blockGetType(block);
    WXR_RSSwizzleImpFactoryBlock dummyFactory = ^id(WXR_RSSwizzleInfo *swizzleInfo) {
        return nil;
    };
...
#pragma mark └ WXR_RSSwizzleInfo
typedef IMP (^WXR_RSSwizzleImpProvider)(void);

@interface WXR_RSSwizzleInfo ()
@property (nonatomic, copy) WXR_RSSwizzleImpProvider impProviderBlock;
@property (nonatomic, readwrite) SEL selector;
@end

@implementation WXR_RSSwizzleInfo

- (WXR_RSSwizzleOriginalIMP)getOriginalImplementation {
    NSAssert(_impProviderBlock, nil);
    // Casting IMP to WXR_RSSwizzleOriginalIMP to force user casting.
    return (WXR_RSSwizzleOriginalIMP)_impProviderBlock();
}

@end


#pragma mark └ WXR_RSSwizzle
@implementation WXR_RSSwizzle

static void swizzle(Class classToSwizzle,
    SEL selector,
    WXR_RSSwizzleImpFactoryBlock factoryBlock) {
    ...
    WXR_RSSwizzleImpProvider originalImpProvider = ^IMP {
    ...
    WXR_RSSwizzleInfo *swizzleInfo = [WXR_RSSwizzleInfo new];
...
+ (BOOL)swizzleInstanceMethod:(SEL)selector
                ...
                newImpFactory:(WXR_RSSwizzleImpFactoryBlock)factoryBlock
                         mode:(WXR_RSSwizzleMode)mode
                          key:(const void *)key {
    NSAssert(!(NULL == key && WXR_RSSwizzleModeAlways != mode),
        @"Key may not be NULL if mode is not WXR_RSSwizzleModeAlways.");

    @synchronized(swizzledClassesDictionary()) {
        if (key) {
            NSSet *swizzledClasses = swizzledClassesForKey(key);
            if (mode == WXR_RSSwizzleModeOncePerClass) {
                if ([swizzledClasses containsObject:classToSwizzle]) {
                    return NO;
                }
            } else if (mode == WXR_RSSwizzleModeOncePerClassAndSuperclasses) {
